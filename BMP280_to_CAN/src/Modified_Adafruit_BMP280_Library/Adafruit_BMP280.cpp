...
/*!
 * Reads the barometric pressure from the device.
 * @return Barometric pressure in Pa as an unsigned integer
 */
uint32_t Adafruit_BMP280::readPressureInt(){
  if (!_sensorID)
  ...
  // Read raw pressure from the sensor
  int32_t adc_P = read24(BMP280_REGISTER_PRESSUREDATA);
  adc_P >>= 4;  // TODO is this necessary for int?

  // Calculate pressure (based on the code in the datasheet)
  int64_t var1, var2, p;
